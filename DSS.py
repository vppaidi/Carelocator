    ...
    if request.method == 'POST':
        ...
        if not file:
            if selected_dropdown == 'Sweden':
                ...
                file_path = f"{selected_dropdown}.csv"
        ...
        # when file is uploaded
            ...
            if selected_dropdown == 'Sweden':
               ...
               result = locations[locations['Name'] == selected_dropdown][['Latitude', 'Longitude']].reset_index(drop=True)
               for idx, (latitude, longitude) in enumerate(result.values):
                   latitude_str = str(latitude).strip()
                   longitude_str = str(longitude).strip()
                   idx=idx+num_rows
                   addresses.append({'index': idx, 'lat': latitude_str, 'lon': longitude_str})
